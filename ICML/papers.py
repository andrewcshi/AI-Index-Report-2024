...
def find_papers(file, keywords):
  ...
  with open(file, 'r') as facct_file:
    ...
    papers = soup.find_all('a', href=lambda x: x and '/virtual/2020/' in x)
  
  # Iterate over the found papers
  for paper in papers:
      paper_lower = paper.text.lower()
      link = 'https://icml.cc' + paper['href']
      abstract = get_abstract(link)
      for keyword in keywords:
          if keyword in paper_lower or keyword in abstract: # Check if keyword match exists
            papers_links[link] = paper.text # Store link and title in dictionary
            print('Executed')
...
### TESTING ###
result = find_papers('icmlpapers20.html', security_keywords)
print(result)